...
class CourseTeamPageTest(StudioCourseTest):
    ...
    def _update_user(self, **update_values):
        """
        Update other_user with provided values

        Arguments:
            `update_values`: dictionary containing values of attributes to be updated
        """
        AutoAuthPage(
            self.browser, no_login=True, username=self.other_user.get('username'),
            email=self.other_user.get('email'), password=self.other_user.get('password'),
            is_active=update_values.get('is_active')
        ).visit()
    ...
    def test_admins_can_delete_other_inactive_users(self):
        ...
        # inactivate user
        self._update_user(is_active=False)
        self.page.delete_user_from_course(self.other_user.get('email'))
